defmodule Waverider.FileTest do
  ...
  test ".construct/2 builds a File struct and updates attributes set in map", %{binary: binary, struct_stub: struct_stub} do
    assert Waverider.File.construct(binary, %{num_channels: 12}) == struct_stub |> Map.put(:num_channels, 12)
  end
  ...
  test ".write/2 writes a file struct to disk as binary data", %{binary: binary, struct_stub: struct_stub} do
