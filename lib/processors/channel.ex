...
defmodule Waverider.Processor.Channel do
  ...
  defp split_stereo([byte_one, byte_two, byte_three, byte_four | tail], channel_one, channel_two, struct) when is_map(struct) do
  ...
  defp split_stereo([], channel_one, channel_two, struct) when is_map(struct) do
   [first_channel | [second_channel]] = Enum.map(
  ...
  defp return_binary_data_from_list(data) do
